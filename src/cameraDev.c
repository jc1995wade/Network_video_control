...
void init_fmt()
{
    ...
    fmt.fmt.pix.width = 680;//图像的宽度
    fmt.fmt.pix.height = 480;//图像的高度
...
{
//	char tpf[20] = {'\0'};
//	sprintf(tpf, "jpg/Image_%d.jpg", counter++);
	lock_jpg(&jpg_file); // 上锁
	jpg_file.fd = open("jpg/Image_7.jpg", O_WRONLY|O_CREAT|O_TRUNC, 0644);
		suc_err(jpg_file.fd, "open");
	print_picture(jpg_file.fd, buffer[okindex].start, buffer[okindex].length);
	close(jpg_file.fd);
